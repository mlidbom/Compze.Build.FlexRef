...
internal static class FlexReferenceResolutionAssertion
{
    public static void AssertMsBuildResolvesReferencesBasedOnSolutionMembership(DirectoryInfo workspaceDirectory)
    {
        var flexRefPropsFile = new FileInfo(Path.Combine(workspaceDirectory.FullName,
            DomainConstants.BuildDirectoryName, DomainConstants.PropsFileName));
    ...
    private static HashSet<string> ParseSlnxProjectFileNames(FileInfo slnxFile)
    {
        return XDocument.Load(slnxFile.FullName)
            .Descendants("Project")
    ...
    private static List<string> FindFlexReferencedPackageIds(Project msbuildProject)
    {
        return msbuildProject.Properties
            .Where(property => property.Name.StartsWith(DomainConstants.UsePackageReferencePropertyPrefix,
                StringComparison.OrdinalIgnoreCase))
            .Select(property =>
                property.Name[DomainConstants.UsePackageReferencePropertyPrefix.Length..].Replace('_', '.'))
            .ToList();
    }
}